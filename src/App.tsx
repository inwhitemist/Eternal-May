...
const api = {
  ...
  login: (email: string, password: string) =>
    http<{ user: MeUser }>("/api/auth/login", { method: "POST", body: JSON.stringify({ email, password }) }),
  logout: () => http<{ ok: true }>("/api/auth/logout", { method: "POST" }),
  register: (email: string, password: string, invite: string) =>
    http<{ user: MeUser }>("/api/auth/register", { method: "POST", body: JSON.stringify({ email, password, invite }) }),
...
function toICS(events: EventItem[]) {
  const lines = ["BEGIN:VCALENDAR","VERSION:2.0","PRODID:-//Life Timeline//EN"];
...
const SAMPLE: EventItem[] = [
  { id: uid(), date: "2019-07-12", title: "Первое большое путешествие", description: "Море, фото на плёнку и чувство полной свободы.", tags: ["путешествия","лето"], color: "#60a5fa" },
  { id: uid(), date: "2021-11-05", title: "Первый релиз собственного плагина", description: "Долгие ночи за клавиатурой окупились.", tags: ["код","релиз"], color: "#34d399" },
  { id: uid(), date: "2023-02-14", title: "Особенный день", description: "Кофе, снег и тёплый разговор.", tags: ["личное"], color: "#f472b6" },
];
...
function Button({ className, children, onClick, variant = "primary", type = "button", disabled, }:
  React.PropsWithChildren<{ className?: string; onClick?: () => void; variant?: "primary" | "ghost" | "outline" | "soft"; type?: "button" | "submit" | "reset"; disabled?: boolean; }>
) {
  const base = "inline-flex items-center gap-2 rounded-2xl px-4 py-2 text-sm font-medium shadow-sm transition active:scale-[.98]";
  const variants: Record<string, string> = {
    primary: "bg-gradient-to-br from-indigo-500 to-fuchsia-500 text-white hover:opacity-95",
    ghost: "bg-transparent hover:bg-white/10 text-foreground dark:text-white border border-transparent",
  ...
  return (
    <button type={type} disabled={disabled} onClick={onClick} className={cn(base, variants[variant], disabled && "opacity-60 cursor-not-allowed", className)}>
      {children}
    </button>
  );
}
...
function Chip({ selected, label, onClick }: { selected?: boolean; label: string; onClick?: () => void }) {
  return (
    <button onClick={onClick} className={cn("px-3 py-1 rounded-full text-xs border transition",
      selected ? "bg-indigo-500/90 text-white border-transparent" : "bg-white/70 dark:bg-white/5 backdrop-blur border-black/10 dark:border-white/10 hover:bg-white")}>
      {label}
    </button>
  );
}
...
function Input(props: React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input {...props} className={cn("w-full rounded-xl border border-black/10 dark:border-white/10 bg-white/80 dark:bg-white/5 px-3 py-2 text-sm outline-none",
      "focus:ring-2 focus:ring-indigo-400/60", props.className)} />
  );
}

function Textarea(props: React.TextareaHTMLAttributes<HTMLTextAreaElement>) {
  return (
    <textarea {...props} className={cn("w-full rounded-xl border border-black/10 dark:border-white/10 bg-white/80 dark:bg-white/5 px-3 py-2 text-sm outline-none",
      "focus:ring-2 focus:ring-indigo-400/60", props.className)} />
  );
}
...
function AuroraLayer() {
  ...
  useEffect(() => {
    let mounted = true, mx = 0, my = 0, tx = 0, ty = 0;
    const lerp = (a:number,b:number,t:number)=>a+(b-a)*t;
    const onPointer = (e: PointerEvent) => { const r = ref.current?.getBoundingClientRect(); if (!r) return;
      tx = ((e.clientX - r.left)/r.width - .5) * 2; ty = ((e.clientY - r.top)/r.height - .5) * -2; };
    const onOrient = (e: DeviceOrientationEvent) => { const g = (e.gamma ?? 0) / 45; const b = (e.beta ?? 0) / 45; tx = g; ty = -b; };
    const mq = window.matchMedia('(prefers-reduced-motion: reduce)');
    let reduced = mq.matches; const onMQ = () => { reduced = mq.matches; }; mq.addEventListener?.('change', onMQ);
    let raf = 0; const tick = () => { if (!mounted) return; if (!reduced) { mx = lerp(mx, tx, 0.08); my = lerp(my, ty, 0.08); } else { mx=my=tx=ty=0; }
      ref.current?.style.setProperty('--mx', String(mx)); ref.current?.style.setProperty('--my', String(my)); raf = requestAnimationFrame(tick); };
    raf = requestAnimationFrame(tick);
    window.addEventListener('pointermove', onPointer, { passive: true });
    window.addEventListener('deviceorientation', onOrient);
    return () => { mounted = false; cancelAnimationFrame(raf);
      window.removeEventListener('pointermove', onPointer); window.removeEventListener('deviceorientation', onOrient); mq.removeEventListener?.('change', onMQ); };
  }, []);
  return (
    <div ref={ref} aria-hidden className="pointer-events-none absolute inset-0 -z-0 overflow-hidden"
      style={{['--c1' as any]:'rgba(139, 92, 246, 0.28)',['--c2' as any]:'rgba(59, 130, 246, 0.24)',['--c3' as any]:'rgba(236, 72, 153, 0.22)'}}>
      {[{s:800,b:130,o:.65,c:'var(--c1)',dx:90,dy:40},{s:620,b:110,o:.60,c:'var(--c2)',dx:-120,dy:25},{s:700,b:120,o:.50,c:'var(--c3)',dx:15,dy:-70}]
        .map((m,i)=>(<span key={i} className="absolute rounded-full mix-blend-screen"
          style={{width:m.s,height:m.s,left:'45%',top:'30%',
            transform:`translate3d(calc(-50% + var(--mx)*${m.dx}px), calc(-50% + var(--my)*${m.dy}px), 0)`,
            background:`radial-gradient(50% 50% at 50% 50%, ${m.c}, transparent 60%)`, filter:`blur(${m.b}px)`, opacity:m.o}}/>))}
...
export default function LifeTimelineApp() {
  ...
  async function refreshMe(){ try{ const r = await api.me(); setMe(r.user);} catch{ setMe(null);} }
  async function refreshEvents(){
    try{ const r = await api.getEvents(); setEvents(r.events || []); }
    catch{ setEvents(SAMPLE); /* fallback чтобы не пусто */ }
  }

  useEffect(()=>{ refreshMe(); refreshEvents(); },[]);
  ...
  const filtered = useMemo(() => {
    ...
    return events
      ...
      .filter((e) =>
        !q ? true : [e.title, e.description, e.tags.join(" ")].filter(Boolean).some((s) => s!.toLowerCase().includes(q))
      )
  ...
  useEffect(() => {
    function onKey(e: KeyboardEvent) {
      ...
      if (e.key === "ArrowDown") { e.preventDefault(); const next = Math.min(items.length - 1, activeIndex + 1); (items[next] || items[0])?.focus(); }
      else if (e.key === "ArrowUp") { e.preventDefault(); const prev = Math.max(0, activeIndex - 1); (items[prev] || items[items.length - 1])?.focus(); }
    }
  ...
  function upsertEvent(ev: EventItem) {
    (async () => {
      try {
        ...
        await refreshEvents();
      } catch (e:any) { alert("Ошибка сохранения: " + (e?.message || "unknown")); }
    })();
  }
  ...
  function clearAll() {
    ...
    (async () => {
      try { await Promise.all(events.map(e => api.deleteEvent(e.id))); await refreshEvents(); }
      catch (e:any) { alert("Ошибка очистки: " + (e?.message || "unknown")); }
    })();
  }
    ...
    reader.onload = async () => {
      try {
        ...
        if (Array.isArray(data)) {
          ...
          for (const ev of valid) { try { await api.createEvent(ev as any); } catch {} }
          await refreshEvents();
        } else { alert("Ожидался массив событий"); }
  ...
  function handleLogin() {
    (async () => {
      ...
      try {
        ...
        const want = confirm("Пользователь не найден. Зарегистрировать админа по инвайту?");
        if (!want) return;
        const invite = prompt("Инвайт-код") || "";
        if (!invite) return;
        try { await api.register(email, password, invite); await refreshMe(); alert("Готово"); }
        catch(e2:any){ alert("Ошибка регистрации: " + (e2?.message || "unknown")); }
      }
    })();
  }
  function handleLogout(){ (async()=>{ try{ await api.logout(); } finally { setMe(null);} })(); }
  ...
  function EventForm({ initial, onSubmit, onCancel }: { initial?: Partial<EventItem>; onSubmit: (e: EventItem) => void; onCancel: () => void }) {
    ...
    return (
      <form onSubmit={(e) => { e.preventDefault();
          const ev: EventItem = { id: (initial?.id as string) || uid(), date, title: title.trim() || "Без названия",
            description: description.trim(), tags, color: color || undefined, imageData, };
          onSubmit(ev); }} className="flex min-h-0 flex-1 flex-col">
        <div className="grid max-h-[60vh] gap-3 overflow-y-auto pr-1">
          <div className="grid gap-1"><label className="text-xs text-black/60 dark:text-white/60">Дата</label>
            <Input type="date" value={date} onChange={(e) => setDate(e.target.value)} required /></div>
          <div className="grid gap-1"><label className="text-xs text-black/60 dark:text-white/60">Заголовок</label>
            <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Например: Переезд в новый город" /></div>
          <div className="grid gap-1"><label className="text-xs text-black/60 dark:text-white/60">Описание</label>
            <Textarea rows={4} value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Короткая история/заметки" /></div>
          <div className="grid gap-1"><label className="text-xs text-black/60 dark:text-white/60">Теги</label>
            <div className="flex gap-2"><Input value={tagInput} onChange={(e) => setTagInput(e.target.value)} placeholder="Добавить тег"
              onKeyDown={(e) => e.key === "Enter" && (e.preventDefault(), addTag())} />
              <Button variant="soft" onClick={addTag}><Tag size={16} />Добавить</Button></div>
            <div className="flex flex-wrap gap-2 pt-1">{tags.map((t) => (<Chip key={t} label={t} onClick={() => setTags(tags.filter((x) => x !== t))} />))}</div></div>
          <div className="grid gap-3"><label className="text-xs text-black/60 dark:text-white/60">Цвет акцента</label>
            <div className="flex flex-wrap items-center gap-3">{["#60a5fa","#34d399","#f472b6","#fb923c","#a78bfa","#fbbf24"].map((c)=>(
              <button key={c} type="button" onClick={()=>setColor(c)}
                className={cn("h-6 w-6 rounded-full border border-black/20 dark:border-white/10", color===c && "ring-2 ring-offset-2 ring-indigo-400")}
                style={{background:c}} aria-label={`Выбрать цвет ${c}`} />))}</div></div>
          <div className="grid gap-3"><label className="text-xs text-black/60 dark:text-white/60 flex items-center gap-2"><ImageIcon size={16}/> Фото (необязательно, 1 шт)</label>
            <input type="file" accept="image/*" onChange={onFile} className="text-xs" />
            {imageData && (<div className="mt-2 overflow-hidden rounded-xl border border-black/10 dark:border-white/10">
              <img src={imageData} alt="Превью" className="max-h-48 w-full object-cover" /></div>)}
          </div>
        </div>
  ...
  function EventCard({ ev, className = "" }: { ev: EventItem; className?: string }) {
    ...
    return (
      <motion.button layout data-timeline-card tabIndex={0}
        onClick={() => { setSelected(ev); setDetailOpen(true); }}
        initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -10, opacity: 0 }}
        className={cn("group relative w-full text-left rounded-3xl border border-black/5 p-5 shadow-lg backdrop-blur transition hover:shadow-2xl focus:outline-none focus:ring-2 focus:ring-indigo-300",
          "bg-white/70 dark:bg-white/5", className)}
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="text-base font-semibold text-neutral-900 dark:text-white sm:text-lg">{ev.title}</div>
          {admin && (
            <div className="flex items-center gap-2 opacity-90" onClick={(e) => e.stopPropagation()}>
              <Button variant="soft" onClick={() => { setEditing(ev); setDialogOpen(true); }}><Edit3 size={16} /></Button>
              <Button variant="outline" onClick={() => deleteEvent(ev.id)}><Trash2 size={16} /></Button>
            </div>
          )}
        </div>
        ...
        {ev.description && (<p className="pt-3 text-sm leading-relaxed text-neutral-800 dark:text-neutral-200">{ev.description}</p>)}
        {ev.tags?.length ? (
          <div className="mt-3 flex flex-wrap gap-2">
            {ev.tags.map((t) => (<span key={t} className="rounded-full bg-indigo-500/10 px-2 py-0.5 text-xs text-indigo-700 dark:text-indigo-300" style={{ border: `1px solid ${accent}55` }}>#{t}</span>))}
          </div>
        ) : null}
      </motion.button>
    );
  ...
  function MonthGrid() {
    const grouped: Record<number, EventItem[]> = {}; for (let i = 0; i < 12; i++) grouped[i] = [];
        ...
        {MONTHS.map((m, i) => (
          <div key={m} className="rounded-3xl border border-black/10 bg-white/60 p-4 dark:border-white/10 dark:bg-white/5">
            ...
            <div className="grid gap-2">
              {grouped[i].length ? grouped[i].map((ev) => <EventCard key={ev.id} ev={ev} />)
  ...
  function prevYear() { if (currentYearIndex > 0) setYear(years[currentYearIndex - 1]); }
  function nextYear() { if (currentYearIndex < years.length - 1) setYear(years[currentYearIndex + 1]); }
    ...
    <div className="min-h-dvh bg-gradient-to-br from-indigo-50 via-white to-rose-50 text-neutral-900 transition dark:from-[#0B0B12] dark:via-[#0B0B12] dark:to-[#14121B] dark:text-white">
      ...
      <header className="relative isolate min-h-[100svh] overflow-hidden">
        ...
        <div className="absolute inset-x-0 top-0 z-20">
          <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4">
            ...
            <div className="relative flex items-center gap-2">
              <Button variant="soft" onClick={() => setDark((d) => !d)}>{dark ? <SunMedium size={16} /> : <MoonStar size={16} />} Тема</Button>
              <div className="relative">
                ...
                {settingsOpen && (
                  <div className="absolute right-0 z-30 mt-2 w-56 overflow-hidden rounded-2xl border border-black/10 bg-white/95 p-2 shadow-xl backdrop-blur dark:border-white/10 dark:bg-neutral-900/95">
                    <button className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-sm hover:bg-black/5 dark:hover:bg-white/10" onClick={() => downloadFile("events.json", JSON.stringify(events, null, 2))}><FileJson size={16}/> Экспорт JSON</button>
                    <button className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-sm hover:bg-black/5 dark:hover:bg-white/10" onClick={() => downloadFile("events.ics", toICS(events), "text/calendar")}><FileDown size={16}/> Экспорт iCal</button>
                    {admin && (
                      <>
                        ...
                        <button className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-sm text-red-600 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-900/20" onClick={clearAll}><Trash2 size={16}/> Очистить всё</button>
                      </>
                    )}
                  </div>
                )}
        ...
        <div className="relative z-10 mx-auto flex min-h-[100svh] max-w-6xl flex-col items-center justify-center px-4 text-center">
          <motion.h1 initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} transition={{duration:.6}} className="text-4xl font-black tracking-tight sm:text-6xl md:text-7xl">Экскурсия в мой мир</motion.h1>
          ...
          <div className="pointer-events-none absolute inset-x-0 bottom-6 z-10 flex items-center justify-center">
            <Button className="pointer-events-auto backdrop-blur" variant="soft" onClick={scrollToTimeline}><ArrowDown size={16}/> Перейти к событиям</Button>
          </div>
        </div>
      </header>
        ...
        <section className="-mt-6 mb-6 rounded-3xl border border-black/10 bg-white/70 p-4 shadow-xl backdrop-blur dark:border-white/10 dark:bg-white/5">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex flex-1 items-center gap-2">
              ...
              <Button variant="soft" onClick={() => { setActiveTags([]); setYear("all"); setMonth("all"); setQuery(""); }}><Filter size={16} /></Button>
              {admin && (<Button onClick={() => { setEditing(null); setDialogOpen(true); }}><Plus size={16}/> Новое</Button>)}
            </div>
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-3">
            <div>
              ...
              <div className="flex flex-wrap gap-2"><Chip label="Все" selected={year === "all"} onClick={() => setYear("all")} />{years.map((y) => (<Chip key={y} label={String(y)} selected={year === y} onClick={() => setYear(y)} />))}</div>
            </div>
            <div>
              <div className="mb-2 text-xs font-semibold uppercase tracking-wide opacity-70">Месяцы</div>
              <div className="flex flex-wrap gap-2"><Chip label="Все" selected={month === "all"} onClick={() => setMonth("all")} />{MONTHS.map((m, i) => (<Chip key={m} label={m.slice(0, 3)} selected={month === i} onClick={() => setMonth(i)} />))}</div>
            </div>
            {allTags.length ? (
              <div>
                <div className="mb-2 text-xs font-semibold uppercase tracking-wide opacity-70">Теги</div>
                <div className="flex flex-wrap gap-2">{allTags.map((t) => (<Chip key={t} label={t} selected={activeTags.includes(t)} onClick={() => setActiveTags((prev) => prev.includes(t) ? prev.filter((x) => x !== t) : [...prev, t])} />))}</div>
              </div>
            ) : <div/>}
          </div>
        </section>
        ...
        <AnimatePresence mode="popLayout">
          {view === "timeline" ? (
            <motion.div ref={listRef} layout className="relative grid gap-5 sm:gap-6 md:grid-cols-2">
              {filtered.length ? filtered.map((ev, idx) => (
                <EventCard key={ev.id} ev={ev} className={cn(idx % 2 === 1 && "md:-translate-y-2","transition-transform")} />
              )) : (
      ...
      <AnimatePresence>
        <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)}>
          <div className="sticky top-0 z-10 flex items-center justify-between gap-3 border-b border-black/10 bg-white/95 px-4 py-3 dark:border-white/10 dark:bg-neutral-900/95">
            <div className="flex items-center gap-2 text-lg font-semibold"><Wand2 size={18} /> {editing ? "Редактировать событие" : "Новое событие"}</div>
            <Button variant="ghost" onClick={() => setDialogOpen(false)}><X size={18} /></Button>
          </div>
          <div className="px-4">
            <EventForm initial={editing || undefined}
              onCancel={() => setDialogOpen(false)}
              onSubmit={(ev) => { upsertEvent(ev); setDialogOpen(false); setEditing(null); }} />
          </div>
        </Dialog>
      </AnimatePresence>
      ...
      <AnimatePresence>
        <Dialog open={detailOpen} onClose={() => setDetailOpen(false)}>
          {selected && (
            <>
              <div className="sticky top-0 z-10 flex items-center justify-between gap-3 border-b border-black/10 bg-white/95 px-4 py-3 dark:border-white/10 dark:bg-neutral-900/95">
                <div className="flex items-center gap-2 text-lg font-semibold"><Info size={18} /> {selected.title}</div>
              ...
              <div className="grid max-h-[85vh] gap-3 overflow-y-auto p-4">
                ...
                {selected.imageData && (<div className="overflow-hidden rounded-2xl border border-black/10 dark:border-white/10">
                  <img src={selected.imageData} alt={selected.title} className="max-h-[60vh] w-full object-cover" /></div>)}
                {selected.description && (<p className="text-sm leading-relaxed text-neutral-800 dark:text-neutral-200">{selected.description}</p>)}
                {selected.tags?.length ? (<div className="flex flex-wrap gap-2">
                  {selected.tags.map((t) => (<span key={t} className="rounded-full border border-black/10 bg-black/5 px-2 py-0.5 text-xs dark:border-white/10 dark:bg-white/10">#{t}</span>))}
                </div>) : null}
                {admin && (
      ...
      <footer className="mx-auto max-w-6xl px-4 pb-10 pt-8 text-xs text-neutral-500">
        Сделано с ❤️ — персональная «экскурсия» по важным моментам. Теперь с серверной авторизацией и хранилищем.
      </footer>
    </div>
  );
}
